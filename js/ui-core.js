...
export function render(){
  ...
  document.querySelectorAll('.pill-item, .fab-add').forEach(b=>b.classList.toggle('active', b.dataset.tab===store.currentTab));
...
export function renderHomeCalendarModal(){
  ...
  for(let d=1; d<=daysInMonth; d++){
    ...
    cells += `<div class="cal-day ${isToday?'today':''} ${isSelected?'cal-day-picked':''}" data-pick-date="${dateStr}">${d}</div>`;
  }

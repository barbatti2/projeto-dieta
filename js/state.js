...
export async function syncFromCloud(){
  if(!window.__firestoreReady){
    showToast('Não conectado ao Firestore no momento — não dá pra sincronizar agora.');
    return false;
  }
  delete profileCache[store.currentProfile];
  await loadProfile(store.currentProfile);
...
export async function loadProfile(profile){
  ...
  if(window.__firestoreReady){
    try{
      ...
      if(doc.exists){
        window.__debugDocStatus = 'encontrado';
        ...
        window.__debugDocStatus = 'NÃO encontrado (doc.exists = false)';
        showToast('Atenção: nenhum dado encontrado no Firestore para "'+profile+'" — abrindo em branco.');
      }
    }catch(e){ window.__debugDocStatus = 'erro: '+e.message; reportFirestoreError(e); }
  }
  if(hasLocalStorage()){
    try{
...
export async function saveState(){
  if(store.profileLoading){
    // o perfil ainda está trocando/carregando — NUNCA salvar agora, senão os dados
    // do perfil antigo (ainda em store.state) seriam gravados no documento do perfil novo.
    console.warn('[NutriRing] Salvamento ignorado: troca de perfil em andamento.');
    showToast('Aguarde o perfil carregar antes de salvar.');
    return;
  }
  ...
  if(window.__firestoreReady){
    try{ await window.__db.collection('nutriring').doc(store.currentProfile).set(store.state, {merge:true}); }
